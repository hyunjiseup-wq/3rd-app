...
| 장비 비교 | front_grip / stock / 조준경 사용 여부별 정확도·속도 |
| **파츠 교환 비교** | **파츠 교환 여부별 명중률·초당 발사 수·탄착군 비교 (더미 데이터 예시)** |
...
- 에어소프트 장비 파츠 교환 여부별 성능 지표 비교 (더미 데이터 기반 예시)
- 파츠 구성 유형(순정/일부 교환/다수 교환)에 따른 명중률·초당 발사 수·탄착군 비교
- 조준경 사용 여부와 파츠 교환 여부의 조합 분석
- 엑셀 다중 업로드 및 한글 UI 지원
...
- `part_replaced_any=True` 이면 accuracy_pct 아주 소폭 상승 가능 (더미 가정)
- `barrel_replaced=True` 이면 avg_group_size_cm 아주 소폭 감소 가능 (더미 가정)
- `motor_replaced=True` 또는 `battery_changed=True` 이면 shots_per_sec 아주 소폭 상승 가능 (더미 가정)

> **주의사항:**
> - 파츠 교환 효과는 더미 데이터 생성을 위한 약한 가정이며 실제 성능 검증 결과가 아닙니다.
> - 이 프로젝트는 에어소프트 스포츠/취미 데이터 시각화 예시이며, 실제 화기 개조·위력 증가·불법 무기 제작 정보를 다루지 않습니다.
